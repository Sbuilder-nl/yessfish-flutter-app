      ...
      body: _loading ? const Center(child: CircularProgressIndicator()) : _items.isEmpty ? Center(child: Text(context.tr('tournaments.empty'), style: const TextStyle(color: Colors.black45))) : TourAnker(id: 'toernooi-lijst', child: ListView.builder(
        ...
        itemBuilder: (_, i) {
          ...
          return Card(margin: const EdgeInsets.only(bottom: 10), child: ExpansionTile(
            ...
            children: [
              if (t['status'] == 'open') Padding(padding: const EdgeInsets.symmetric(horizontal: 16), child: Align(alignment: Alignment.centerLeft, child: FilledButton(onPressed: () async { final m = ScaffoldMessenger.of(context); try { await Api.post('/tournaments/$id/join'); m.showSnackBar(SnackBar(content: Text(context.tr('tournaments.joined')))); } catch (e) { m.showSnackBar(SnackBar(content: Text(e is ApiException ? e.message : context.tr('common.error')))); } }, child: Text(context.tr('tournaments.join'))))),
