...
class _MapScreenState extends State<MapScreen> {
  ...
  Future<void> _loadWaters() async {
    try {
      ...
      final w = await Api.get('/waters?lang=$loc8&minLat=${b.south}&minLng=${b.west}&maxLat=${b.north}&maxLng=${b.east}');
  ...
  Future<void> _addWater(LatLng pos) async {
    ...
    try {
      await Api.post('/waters', {'name': name.text.trim(), 'type': type, 'latitude': pos.latitude, 'longitude': pos.longitude});
      await _load();
  ...
  Widget _waterPin(String level) {
    ...
    return Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      Transform.translate(offset: const Offset(0, -4), child: Icon(Icons.arrow_drop_down, color: c, size: 18)),
    ]);
  }
  ...
  Widget build(BuildContext context) {
    ...
    final markers = <Marker>[
      // Bekende waters — kleur naar drukte (vanaf zoom 7, anders te veel).
      if (_zoom >= 7) ..._waters.where((w) => w['latitude'] != null).map((w) => Marker(
        point: LatLng(double.parse('${w['latitude']}'), double.parse('${w['longitude']}')),
        width: 44, height: 48,
        child: GestureDetector(behavior: HitTestBehavior.opaque, onTap: () => _showWater(w as Map), child: _waterPin('${w['busyness']?['level'] ?? 'none'}')))),
    ...
    return Scaffold(
      ...
      floatingActionButton: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.end, children: [
        FloatingActionButton.small(
          ...
          onPressed: () => _addWater(_map.camera.center),
