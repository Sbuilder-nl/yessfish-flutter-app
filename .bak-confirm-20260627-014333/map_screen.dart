...
class _MapScreenState extends State<MapScreen> {
  ...
  Future<void> _toggleAuto() async {
    ...
    try {
      ...
      if (newVal) { await _autoCheckin(); } else { await Api.delete('/checkin'); _checkedIn = false; }
      await _load();
  ...
  void _showWater(Map w) {
    ...
    showModalBottomSheet(context: context, builder: (_) => Padding(padding: const EdgeInsets.all(20),
      child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [Icon(Icons.water, color: _waterColor(level)), const SizedBox(width: 8), Expanded(child: Text(w['name'] ?? '', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)))]),
