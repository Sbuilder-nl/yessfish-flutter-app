...
class _MapScreenState extends State<MapScreen> {
  ...
  Future<void> _init() async {
    ...
    // Houd drukte + eigen status actueel zolang de kaart open is.
    _timer = Timer.periodic(const Duration(seconds: 150), (_) async { await _autoCheckin(); await _load(); });
  }

  // Automatisch in-/uitchecken o.b.v. GPS — alleen als auto AAN staat. Server
  // checkt of je bij water bent (anders gebeurt er niets / word je uitgecheckt).
  Future<void> _autoCheckin() async {
    if (!_autoOn) return;
    try {
      final p = await loc.currentLocation();
      final r = await Api.post('/checkin/auto', {'latitude': p.lat, 'longitude': p.lng});
      _checkedIn = r is Map && r['checked_in'] == true;
    } catch (_) {}
  }
  ...
  // Auto-inchecken aan/uit zetten (privacy-opt-out, blijft bewaard).
